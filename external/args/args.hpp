...
auto try_run(rank<1>, T &x, Ts &&...) ARGS_RETURNS(x.run())
    ...
    template <class T, class... Ts>
    auto parse(T &cmd, std::deque<std::string> a, Ts &&...xs) {
  ...
  return args::try_run(rank<2>{}, cmd, xs...);
}

template <class T, class... Ts>
auto parse(std::deque<std::string> a, Ts &&...xs) {
  // TODO: zero initialize T
  T cmd;
  try {
    return args::parse(cmd, std::move(a), xs...);
...
template <class T> auto parse(int argc, char const *argv[]) {
  std::deque<std::string> as(argv + 1, argv + argc);
  return args::parse<T>(as);
}
